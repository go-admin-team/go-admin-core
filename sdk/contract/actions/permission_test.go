...
func TestPermissionScopes(t *testing.T) {
	...
	cases := []struct {
		...
		// H3: dept_path always starts with "/0/", so DeptId 0 must not be
		// allowed to build a pattern that matches every row.
...
func TestIsValidDataScope(t *testing.T) {
	valid := []string{DataScopeAll, DataScopeCustom, DataScopeDept, DataScopeDeptTree, DataScopeSelf}
	for _, s := range valid {
	...
	invalid := []string{"", "0", "6", "all"}
	for _, s := range invalid {
		if IsValidDataScope(s) {
			t.Errorf("IsValidDataScope(%q) = true, want false", s)
		}
	}
}
